...
2. **Ingest.** `ingest_hf.py` generates candidates for the remaining pilot models, staged for review.
   **Done 2026-09-24 (second tranche):** 27 models promoted after Wilson's review (96 models, 33 datasets, 154 edges; the model count is at the top of the 50–100 target, so further growth is a scope decision). Added lineage types the seed lacked: adapters (`adapter_on`: Alpaca-LoRA, Guanaco), a merge (OpenOrca-Platypus2), feedback via a reward-model record (Starling), and dataset-routed distillation (Orca 1/2, Baize-SDF, Evol-Instruct, Tulu). Judgment calls went to the blind three-tier panel (`docs/review.md`): Starling 3/3, Orca-2 2/3, Baize no majority (Wilson chose the `baize-sdf` stub). Open items: MythoLogic/MythoMax are promoted with no edges on purpose (their cards name parents Stemma has not recorded; drawn with a "parents not recorded" marker); Baize v2's `fine_tuned_from llama-7b` is imprecise (v2 descends from Baize v1.5, not in Stemma); `orca-2-data` → GPT-4 rests on the paper naming GPT-4 explicitly for only part of the set; Orca datasets' availability is `unknown`, not `never_released`; MythoMax/Marcoro-style merges need their parents ingested before they carry `merged_from` edges. `ingest_hf.py` had a license-note argument bug, fixed.
2b. **Site.** `stemma.network` hosts both doors: the Graph (records, raw JSON, `llms.txt`) and the Notebook (Track A's path, with Wilson's dated notes), cross-linked. **`docs/site-plan.md` is the score** (three rulings by Wilson, five by Fable, 2026-09-24). Phases 3 and 4 land on this site.
   **Done 2026-09-24:** live at https://stemma.network (Vercel project `stemma`, root `site/`, git-connected: a push to `main` is a production deploy). Beyond the plan: an About page (genealogy vs. taxonomy, the two audiences), a `/records/` index, and a Phase 3 pilot at `/graph/`: the LLaMA family drawn as a manuscript stemma (time down the page, closed models hollow, contamination dashed), with a separate phone layout. Renderer: `site/src/lib/stemma-svg.ts`; its columns are hand-set for that one family (`LANES`), so new records appear on record pages automatically but not in the drawing. **Extended 2026-09-24** to the tranche-2 LLaMA-lineage records (desktop and phone layouts, axis to 2024-01). Records with unrecorded parents are drawn with a dashed stub ending in a hollow "?". Mistral, StarCoder-line and other families still need their own drawings; known limit: a few long dataset lines cross labels on the desktop drawing. **Mistral family added 2026-09-24** as a second drawing on `/graph/` (`#mistral`); the renderer now takes a per-family `Family` config (`site/src/lib/stemma-svg.ts`: lanes, labels, start month, `monthPx`, optional per-edge `elbow`), LLaMA output byte-identical after the refactor. A parent drawn in another family is an "off-family" stub linking to that drawing. Site has an OG card (`site/public/og.png`, a crop of the LLaMA drawing, no counts so it doesn't go stale; regenerate by hand if the drawing changes a lot).
   **Next (Wilson's direction, 2026-09-24):** exhaustiveness is not a goal without funding; depth through the textual-criticism analogy is. `docs/shared-errors.md` is a DRAFT sketch (not adopted) of a Lachmann/Maas-style shared-error method for `inferred_behavior` edges: probes as *loci*, readings, an apparatus, calibration on known lineage first. Its four questions at the end await Wilson; its *(verify)* citations must be checked before anything public relies on them.
